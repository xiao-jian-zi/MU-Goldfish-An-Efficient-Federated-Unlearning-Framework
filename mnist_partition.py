...
shard_nums = 18
split_size = len(mnist_dataset_train) // shard_nums
indices = list(range(0, len(mnist_dataset_train)))
sub_datasets = []
for i in range(shard_nums):
    start_idx = i * split_size
    end_idx = start_idx + split_size if i < shard_nums - 1 else len(mnist_dataset_train)
...
nets = [CNNMnist(args).cuda() for _ in range(shard_nums)]
for net in nets:
    net.load_state_dict(net_test.state_dict())

optimizers = [optim.SGD(nets[k].parameters(), lr=0.001, momentum=0.9, weight_decay=0) for k in range(shard_nums)]
